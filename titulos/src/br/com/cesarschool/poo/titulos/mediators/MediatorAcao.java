...
public class MediatorAcao {
    ...
    // 1. Criação de uma instância estática e privada da própria classe
    private static MediatorAcao instance;

    // 2. Construtor privado para evitar a criação de instâncias fora da classe
    private MediatorAcao() {
        // Inicialização de recursos, se necessário
    }

    // 3. Método público e estático para obter a única instância da classe
    public static MediatorAcao getInstance() {
        if (instance == null) {
            instance = new MediatorAcao(); // Cria a instância apenas se ainda não existir
        }
        return instance;
    }
}
